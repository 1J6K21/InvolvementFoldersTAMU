...
function parseSegment(segment) {
    ...
    function parseFactor() {
        const tk = peek();
        if (!tk)
            return { type: "COURSE", value: "" }; // shouldn't happen
        if (tk === "EXAM") {
            consume();
        ...
        if (/^[A-Z]{2,4}\s?\d{3}$/i.test(tk)) {
            ...
            if (peek() === "/") {
                ...
                const childNodes = courses.map(c => ({ type: "COURSE", value: c }));
        ...
        while (true) {
            ...
            if (tk === "and" || tk === ",") {
                ...
                if (left.type === "AND" && left.children)
                    ...
                    children.push(left);
                if (right.type === "AND" && right.children)
        ...
        while (true) {
            ...
            if (tk === "or") {
                ...
                if (left.type === "OR" && left.children)
                    ...
                    children.push(left);
                if (right.type === "OR" && right.children)
                    children.push(...right.children);
                else
                    children.push(right);
...
export function parsePrereqTextToTree(text) {
    ...
    for (const seg of rawSegments) {
        ...
        if (requireGrade) {
            ...
            const wrapper = { type: subtree.type, children: subtree.children, requireGrade: true };
            ...
            const wrapper = { type: "CONCURRENT", children: subtree.children ?? (subtree.value ? [{ type: "COURSE", value: subtree.value }] : undefined) };
            children.push(wrapper);
        }
        else {
...
export function printNodeTree(node, indent = 0) {
