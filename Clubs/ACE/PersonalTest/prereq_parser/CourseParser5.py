...
def parse_prerequisites(text: str):
    ...
    # STEP 1: Tokenization
    token_pattern = r"(Pre|Grade|or|and|/|cross|concurrent|\.|;|exam|[A-Z]{4}\s\d{3})"
    ...
    buckets = {"Pre": [], "Concurrent": [], "Cross": [], "Classification": []}

    # Add classification requirements (based on keywords in the full text)
    class_group = []
    ...
    for tok in tokens:
        ...
        # Handle punctuation (end of segment)
        if tok in (";", "."):
            commit_group()
            current_bucket = None  # reset bucket after sentence
            logic_operator = None
            continue

        # Match course codes or exam keywords
